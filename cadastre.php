...
/*require_once("banco_teste.php");*/
require_once("tabelas_trabalho.php");

$filmes = filmes_select();
...
$genero = array(
    ...
    array(2, "Masculino"),
    array(3, "Outro"),
    array(4, "Prefiro não dizer")
);

function radio($nome, $dados) {
    $html = "";
    echo"<label id=\"tituloGenero\" for=\"genero\">Gênero</label>";
    for ($i = 0; $i <= count($dados)-1; $i++) {
        $html.= "<div class=\"inputGenero\">";
        $html.= "<input type=\"radio\" name=\"$nome\" value=\"".$dados[$i][0]."\" id=\"$nome".$dados[$i][0]."\">\n";
        $html .= "<label for=\"$nome".$dados[$i][0]."\">".$dados[$i][1]."</label>";
        $html.= "</div>";
    }
...
/*$filmes = array (
    ...
    array(2, "Aventura"),
    array(3, "Comédia"),
    array(4, "Comédia Romântica"),
    array(5, "Comédia Dramática"),
    array(6, "Drama"),
    array(7, "Biográfico"),
    array(8, "Histórico"),
    array(9, "Fantasia"),
    array(10, "Documentário"),
    array(11, "Ficção Científica"),
    array(12, "Musical")
);*/

function checkbox($nome, $dados) {
    $html = "";
    $html.= "<div class=\"inputGenero\">";
    echo "<legend id=\"tituloGenero\">Seus gêneros cinematográficos preferidos:</legend>";
    for ($i = 0; $i <= count($dados)-1; $i++) {
        $html.= "<input type=\"checkbox\" name=\"$nome\" value=\"".$dados[$i][0]."\" id=\"$nome".$dados[$i][0]."\">\n";
        $html .= "<label for=\"$nome".$dados[$i][0]."\">".$dados[$i][1]."</label><br>";
    }
    $html.="</div>";
    return $html;
}

function checkbox_filmes() {
    global $filmes;
    return checkbox("filmes", $filmes);
}
